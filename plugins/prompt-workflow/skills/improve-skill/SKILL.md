...
- **The owning repo's maintenance skills outrank your instinct** — when the repo ships one covering the target, live or committed (step `0c`, load the maintenance conventions), it is the house style for the edit. A target file that carries no authoring meta is usually a deliberate choice, not an omission: do not "restore" a pointer, a sync note, or a rule reminder into it, and do not infer the conventions from the file's contents when a skill states them.
