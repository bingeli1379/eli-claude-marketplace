...
1. **Identify the targets**
   ...
   **A changed bundled reference pulls in its owner — a reference and the steps that drive it are ONE unit.** A criterion, rule, or template edited inside `references/` is consumed by steps living in another file, so auditing the reference alone cannot see the break (a step citing a criterion the edit just renamed, a budget the longer list now blows past). So widen the set — **on both branches, explicitly specified paths and auto-detected ones alike** (an explicit reference path is the common case: it is how `/improve-skill` drives this audit):
   ...
   - **The second hop is scoped, not banned — take the coupled siblings, never the directory.** Pulling in a `SKILL.md` does NOT then pull in that skill's whole `references/`, or a two-line edit snowballs into auditing a plugin. But stopping dead at the owner loses the defect that pair is most likely to carry: a sibling that **divides the work** with the changed file — one stating a budget the other spends, one declaring which of the two owns a case — shares no distinctive token with it, so Lens B's grep cannot reach it either, and the split contradicts itself with nothing in the run able to see it. The candidates are the siblings the owner names — a bounded list, and the owner is already read in full. A candidate is **coupled**, and becomes a **consulted** file rather than a target, when either file cites the other **by filename or by a unit inside it** (a criterion letter, a lens class number, a section name), or when the owner's steps apply the two together in one pass. One grep of the changed file's name across the candidates settles the first half, the owner's own steps settle the second, and a candidate with neither property stays out. **The test runs in both directions** — a changed `SKILL.md` consults the references it is coupled to by exactly this rule, since a split breaks the same way from either side, and only the reference-side branch has an owner to pull in first.
   - **A consulted file is compared, never audited.** This step only selects it; **step 4, hunt the defect classes, is where it is read** and compared against the changed file for their coupling only. A break in that pair is rated in the lens vocabulary and listed under whichever file the fix lands in, while the consulted file's own standing defects are not this run's business. Step 2, the text pass, does not run on it; it pulls in nothing further — **that is where the widening stops**; and it takes no batch slot, riding with the batch that pulled it in. A file already in the set is a target, and is audited once, not twice.
   - Name the pulled-in and the consulted files in `### 各檔` — suffix the path `（帶入）` or `（僅比對）`, and name a consulted file that produced no finding in that section's closing sentence — so the user sees why each was read and at what depth.
...
4. **Hunt the defect classes**
   ...
   Read `${CLAUDE_PLUGIN_ROOT}/skills/review-skill/references/logic-lenses.md` now — it holds Lens A classes 1–9 and 13–14, Lens B classes 10–12, and Lens B's grep-first sweep procedure with its token budget. Work through **every** class for every in-scope target (per the step 1 per-lens scoping), and follow Lens B's sweep method as written: derive distinctive tokens from what the change touched, grep repo-wide, read only the hits. Reading every sibling file is the wrong method — it costs more and finds less than the grep. **The consulted files step 1 selected are the one exception, and a narrow one**: read each against the changed file for their coupling only, because a split whose two halves share no token is exactly what the grep structurally cannot see.
