...
6. **Phase 1 — Sequential single-writer implementation** (MANDATORY):
   ...
   **For each group (in sequence):**
   ...
   b. **Wait** for the agent to complete, then assess its result (signal vocabulary is defined in `skills/agent-guidelines/SKILL.md` → *Signaling Unknowns*). Note: a background agent frequently signals completion only as a runtime **idle/available notification, not an explicit `DONE`** — so confirm completion **against git** (are the group's commits present + tree healthy?), do not block waiting for a DONE message that may never arrive:
      ...
      - **NEEDS** (report contains a `NEEDS:` line): the agent is *paused awaiting an external fact*, NOT failed. Resolve each NEEDS using whatever tools/knowledge YOU (the orchestrator) have — connected MCP servers, lookup tools, project-knowledge skills, or the user — then **resume the SAME agent with `SendMessage`**: its context is intact, so do NOT re-dispatch a fresh agent and do NOT make it redo work. If a resolved fact contradicts an assumption in `design.md` / `tasks.md`, surface it to the user before resuming — a NEEDS can legitimately invalidate part of the plan. **And when your resolution corrects an artifact, correct every artifact that asserts the same thing, not just the one you were reading.** `design.md` records the decision, but qa-engineer verifies against `specs/*/spec.md`, so a ruling that lands only in the design leaves the spec asserting the superseded claim and QA reports a failure that is really your own un-propagated edit. Measured: a ruling that a route change may legitimately turn a status code from 200 into 404 was written into the design's risk list, while the spec scenario still demanded the response be "equivalent to before" — QA flagged it, correctly, one round later. Before resuming the agent, grep the change directory for the claim you just overturned. sdd names no specific resolving tool; use what the environment provides, and if a NEEDS is genuinely unresolvable, ask the user.
...
7. **Phase 2 — Review + Security (parallel), then QA alone** (MANDATORY — do NOT skip):
   ...
   **Why qa-engineer does not overlap the other two.** The other two are strictly read-only, so they fan out safely. qa-engineer is not: proving a guard can actually fail means temporarily breaking the production code it guards, and it may also drop a probe file to observe something the suite cannot reach. That makes the working tree unstable for the length of its run, and a read-only reviewer sharing that tree reads whatever half-applied state it happens to catch. Measured: a security reviewer read a partially applied mutation off a shared tree, ran the suite, and reported a real-but-meaningless blocker — correct `git status`, correct failure count, and no bearing on the committed code. It cost a round-trip plus a report correction, and the reviewer could not have known better: the fact that another agent was authorised to mutate the tree existed only in the orchestrator's head. Give qa-engineer the tree to itself and the whole class disappears; the serialisation costs one wait.
   ...
   **Fix → Re-verify Loop (max 3 rounds):**
   Reviewing is a *read* task, so review-engineer and security-engineer fan out in parallel (above), with qa-engineer alone after them. **Fixing is a *write* task, so it stays single-threaded** — do NOT dispatch fix agents for different layers in parallel; their edits would diverge and collide exactly like parallel Phase 1 writes.
   ...
   2b. **The fix-diff check — read the fix diff yourself before accepting the round — do NOT accept a fix on the agent's report plus green tests.** Take the diff **once per fix agent** — `git diff <that agent's base>..HEAD`, one read covering everything it committed — then tick off each finding you dispatched to it against that single diff, confirming *that specific defect* is gone. One diff per agent, not one per finding: the check is per-finding, the read is not, and re-diffing per item multiplies the most-repeated read in the loop while confirming nothing extra. This is cheap, and it is the highest-yield check in the whole loop, because the failure it catches is invisible to everything else: a fix agent that addresses **part** of a multi-part finding, reports every item `DONE` in good faith, and leaves a passing test suite behind. Nothing else in the pipeline sees it — the tests were green before the fix too, and the reviewers have not run yet.
       ...
       - **Read the diff body, not `--stat`.** A fix agent writing files through tooling can change a file's *encoding* while changing nothing you asked for: a UTF-8 BOM prepended, CRLF flipped to LF wholesale. `git diff --stat` counts lines and stays silent on all of it, so a diff that looks like "1 file, +3 lines" can also carry three bytes at offset zero. It happened twice in one run, and both times the agent reported the BOM as pre-existing — because it checked the *working file*, which by then already had the BOM it had just added. **Verify against the base, not the artifact the claim has already touched**: `git show <base>:<file> | head -c 3` against the same for `HEAD`. Cheap, and the failure it catches reaches downstream — the second occurrence had already forced a consumer in another repo to write a workaround for a BOM this change itself introduced.
