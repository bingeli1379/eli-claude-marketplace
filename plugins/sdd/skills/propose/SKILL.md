...
5. **Exhaustive codebase scan — identify ALL affected files**
   ...
   **Dry-run for config-flip AND type-level changes:** When a change enables/disables rules, flags, or config that will surface new errors (e.g., enabling ESLint rules, turning on `strict` mode, enabling a feature toggle that changes validation), OR when a change modifies type signatures that affect downstream consumers (e.g., removing index signatures, changing type defaults like `any` → `unknown`, narrowing union types, making properties optional via `Partial<T>`), do NOT estimate the violation count by grepping patterns. Instead:
   ...
   2. Run **ALL** the project's verification and lint commands — both `verification_commands` AND `lint_commands` from `config.yaml`; with no config, run the build/test invocation the project's own CI runs, falling back to its conventional entry points. **NEVER hardcode tool-specific commands** like `npx vue-tsc --noEmit`. Type-aware ESLint rules (e.g., `@typescript-eslint/no-unnecessary-type-assertion`) can surface errors that type-check alone misses.
...
6. **Clarify requirements and define feature boundaries**
   ...
   **b. Define boundaries (multi-dimensional)** — draw scope on five axes, not just features:
   ...
   - **Reversibility**: how hard is this decision to change later (easy / hard / irreversible) — drives how careful the upfront clarification needs to be. **Safe-by-default rule (irreversible / destructive operations):** when the change performs an action with no undo — bulk deletion, mass external mutation (e.g. patching/removing remote resources), data purges — the *proposed default* MUST be the safe one: dry-run / preview / explicit-confirm, with the destructive behavior as an opt-in the caller consciously requests (e.g. `dryRun` defaults to `true`). This is the design you propose in the Scope Contract from the start — NOT a hardening to be added later by review. Do not let caller convenience ("they'll just call it directly") set a destructive default; identifying the risk is not enough, the default must encode the safe choice.
     ...
     **When the answer to reversibility is "put it behind a runtime switch", that is a question for the user, not a conclusion for `design.md`.** A feature flag, a global setting, a config kill switch — each one is cheap to write and expensive to own: somebody has to create the key in every environment, decide the order the environments flip in, watch the right dashboard between flips, and know which of the switches retires which half of the change. None of that is work an agent can do, and none of it was in what the user asked for. Measured: a change across four deployments proposed three new runtime switches plus one deliberate no-switch, each with a sound reason, and the user first learned of them in the final handoff — 「我沒說要做 toggle」. The reasoning had been right and the decision had still never been theirs.
     So when you are about to propose one, put it in the **6e chain message** on the `NFR/Data/Reversibility` line, with what it costs them attached: which switch, in which system, who creates it, and what the alternative is (usually: ship it on, and revert the deploy if it misbehaves). One line each. If they say no, the change ships without it and the reversibility axis records that as their call — that is a legitimate answer, and a design that hides an unwanted switch behind good reasoning is worse than one that ships without it. This does **not** apply to a switch the user asked for, nor to one the codebase already has and the change merely reads.
